...
export default function FavoriteScreen() {
  ...
  // Filtra solo gli elementi preferiti per evitare di ripetere il filtro
  const favoriteItems = currentData.filter((item) => item.favorite);
      ...
      <ScrollView className="px-5">
        ...
        {isLoading ? (
          ...
          categories
            .filter(
              (category) =>
                category !== "new_releases" && category !== "recently_played"
            )
            .map((category) => {
              // Filtra gli elementi preferiti in base alla categoria
              const categoryItems = favoriteItems.filter(
